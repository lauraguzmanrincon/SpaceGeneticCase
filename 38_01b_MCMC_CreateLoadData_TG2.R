...
# __.1. Constants ----
# TG2: num periods and blocks are multiplied by numSTGroups
numSTGroups <- length(stGeneticGroups) + 1
minWeek <- min(dataInfoPostSOA$numWeek_corrected)
maxWeek <- max(dataInfoPostSOA$numWeek_corrected)
numBlockPeriodsPerST <- floor((maxWeek - minWeek + 1)/lengthBlockPeriod)
numPeriodsPerST <- numBlockPeriodsPerST*lengthBlockPeriod
numBlockPeriods <- numSTGroups*numBlockPeriodsPerST
numPeriods <- numSTGroups*numPeriodsPerST

# __.2. Create weeksForModels (as weeksInfo in 36a.R/2.) ----
# 30.03.2020 Same as before but using numPeriodsPerST instead of numPeriods
weeksForModelsOld <- data.table(weekId = 1:numPeriodsPerST, key = "weekId")
weeksForModelsOld[order(weekId), row := 1:numPeriodsPerST]
weeksForModelsOld[order(weekId), groupId := ceiling(row/lengthBlockPeriod)]
...
weeksForModelsOld[, numWeek := minWeek + weekId - 1]
weeksForModelsOld[, received_date_nextFriday := tempMinDate + 7*(weekId - 1)]
weeksForModelsOld[, received_date_nextFriday_group := tempMinDate + 7*lengthBlockPeriod*(groupId - 1)]
dataInfoPostSOA[numWeek_corrected == minWeek + numPeriodsPerST - 1, max(received_date_nextFriday)] ==
  weeksForModelsOld[, max(received_date_nextFriday)] # useless?
# 30.03.2020 NEW: merge weeksForModels with short weeksForModelsOld, ignoring row and groupId
weeksForModels <-CJ(STgroupId = 1:numSTGroups, weekId = 1:numPeriodsPerST)
weeksForModels[, row := (STgroupId - 1)*numPeriodsPerST + weekId]
weeksForModels[, max(row) == numPeriods]
setkey(weeksForModels, weekId)
setkey(weeksForModelsOld, weekId)
weeksForModels[weeksForModelsOld, c("groupIdOld", "numWeek", "received_date_nextFriday", "received_date_nextFriday_group") :=
                 .(groupId, numWeek, received_date_nextFriday, received_date_nextFriday_group)]
weeksForModels[, groupId := (STgroupId - 1)*numBlockPeriodsPerST + groupIdOld]
length(unique(weeksForModels$groupId)) == numBlockPeriods
max(weeksForModels$groupId) == numBlockPeriods
...
# __.0A. Create STForModels (NEW 30.03.2020)
STForModels <- data.table(STgroupId = 1:numSTGroups, ccNum = c(stGeneticGroups, NA))

# __.0B. Add ST to dataInfoPostSOA and groupSequencesCore (NEW 30.03.2020)
# Why adding it to groupSequencesCore? We need STgroupId for all G terms, including the empty ones in OX, e.g.
setkey(dataInfoPostSOA, id)
setkey(dataAnnotatedList[[2]], id)
dataInfoPostSOA[dataAnnotatedList[[2]], cc := clonal_complex..MLST.]
dataInfoPostSOA[, ccNum := as.integer(gsub("ST-| complex", "", cc))]
setkey(dataInfoPostSOA, ccNum)
setkey(STForModels, ccNum)
dataInfoPostSOA[STForModels, STgroupId := STgroupId]
dataInfoPostSOA[is.na(STgroupId), STgroupId := numSTGroups]
setkey(groupSequencesCore, columnInDataAlleles)
setkey(dataInfoPostSOA, columnInDataAlleles)
groupSequencesCore[dataInfoPostSOA, c("STgroupId", "ccNum") := .(STgroupId, ccNum)]
...
sum(weeksForModels[, .N, numWeek]$N != 1) == 0 # FALSE now
setkeyv(casesForModels, c("numWeek_corrected", "STgroupId")) # change 30.03.2020
setkeyv(weeksForModels, c("numWeek", "STgroupId")) # change 30.03.2020
...
# __.0C. Add ST to geonmeForModels (NEW 30.03.2020)
#setkey(genomeForModels, col)
#setkey(casesForModels, dim3Cases) # doesnt work
#genomeForModels[casesForModels, STgroupId := STgroupId]
tempTable <- groupSequencesCore[!is.na(clusterLowId), .(min(STgroupId), max(STgroupId)), clusterLowId]
tempTable[, sum(V1!=V2)] == 0
setkey(genomeForModels, clusterLowId)
setkey(tempTable, clusterLowId)
genomeForModels[tempTable, STGroupId := V1]

# --- (30.03.2020) From here nothing changed respect to the TG version --- #
...
# NEW 30.02.2020
# Remove y entries that should not interfere with likelihood calculation
entriesToRemove <- do.call(rbind, lapply(1:numSTGroups, function(ST) CJ(x = (ST - 1)*numPeriodsPerST + 1:numPeriodsPerST,
                                                                        y = 1:numRegions,
                                                                        z = genomeForModels[STGroupId != ST, col])))
prod(dim(y)) - numPeriodsPerST*numSequences == nrow(entriesToRemove)
sum(y[as.matrix(entriesToRemove)]) == 0
y[as.matrix(entriesToRemove)] <- NA
totalCasesInY == sum(y, na.rm = T)

# TODO save new constants
# TODO create precision matrix for R
...
groupSequencesCore_chosen <- groupSequencesCore # 18.12.2019 :) # Very important!
...
save(inputForData,
     ...
     weeksForModels, regionsForModels, genomeForModels, casesForModels, STForModels,
     groupSequencesCore_chosen,
     numSequences, numPeriods, numRegions, numBlockPeriods, numBlockRegions, numBlocksSequences,
     numSTGroups, numPeriodsPerST, numBlockPeriodsPerST, # NEW 30.03.2020
     iToGroups, jToGroups, kToGroups,
     numBeta, pop,
